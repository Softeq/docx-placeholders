...
import net.sl.dto.ImageHolderDto;
import net.sl.exception.DocxTemplateFillerException;
import net.sl.processor.ImageTagProcessor;
...
public class ImageTagProcessorTest {
    ...
    public void testFillingField() {
        try (InputStream templateIs = getClass().getResourceAsStream("/net/sl/ImageTagProcessorTest-fieldRef-template.docx");
             ByteArrayOutputStream filledTemplateOs = new ByteArrayOutputStream();) {

            DocxTemplateFillerContext context = new DocxTemplateFillerContext();
            context.setProcessors(Collections.singletonList(new ImageTagProcessor()));
            ImageDto imageData = new ImageDto("Heart Image", getClass().getResourceAsStream("/net/sl/image/heart.png"), "png", 200, 100);
